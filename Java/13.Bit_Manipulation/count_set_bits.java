...
public class count_set_bits{
    public static void main(String args[]){
        ...
        //First of all set a counter which will count number of setted bits and initialize it to 0.Then perform AND operation b\w given value and 1.Result will be equall to 1 if last bit of n is setted else it will be 0. then check whether result is 0 or not if zero the it means that our LSB was 1  so count it to counter then make a right shift of this number by 1 and then perform the same operation until number becomes zero and then print the counter
        Scanner sc = new Scanner(System.in);
        System.out.print("Enter number to check its number of set bits : ");
        int n=sc.nextInt();
        int counter=0;
        while(n>0){
            int x = n&1;
