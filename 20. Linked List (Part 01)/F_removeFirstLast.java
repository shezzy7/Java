public class F_removeFirstLast{
    ...
    public void removeLast(){
        ...
        //here to find second last node of list we will define two nodes.In which one node will be pointing to head and second will be poing to hed.next.And then we will run loop which in which we will move these two node one step forward each time.And we will go on moving until our second node reaaches at last node and when this second node will reach at last node then our first node which is one step behind from this will be at second lst nast and we will make this seconLast nodes next null.So in this case our last node will be removed from ouur list and will be stored in java garbage collection
